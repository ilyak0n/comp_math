...
st.set_page_config(page_title="Трансформация координат", layout="wide")
...
with col2:
    st.subheader("📊 Предварительный просмотр и отправка")
            ...
            if not required_cols.issubset(df.columns):
                ...
                if st.button("🚀 Запустить трансформацию и скачать DOCX"):
                    with st.spinner("Бэкенд обрабатывает координаты..."):
                        ...
                        try:
                            # Отправляем запрос на локальный FastAPI
                            response = requests.post("http://127.0.0", files=files, data=data)
                            ...
                            st.error(f"Не удалось связаться с бэкендом: {str(e)}")
